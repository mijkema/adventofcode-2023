...
def main(inp, is_real):
    inp = inp.strip().split('\n')
    size_x = len(inp[0])
    grid = [['.' for _ in range(size_x + 2)]]
    for line in inp:
        grid.append(['.'] + list(line) + ['.'])
    grid.append(['.' for _ in range(size_x + 2)])
    ...
    for x, row in enumerate(grid):
        for y, cell in enumerate(row):
            if cell.isnumeric() and start_int == -1:
                start_int = y
            if not cell.isnumeric() and start_int != -1:
                value = int("".join(grid[x][start_int:y]))
                for row_index, to_check_row in enumerate(grid[x-1:x+2]):
                    for cell_index, to_check_cell in enumerate(to_check_row[start_int-1:y+1]):
                        if to_check_cell == '*':
                            gears[((x-1) + row_index, (start_int-1 + cell_index))].append(value)
                start_int = -1
    res = sum(v[0] * v[1] if len(v) == 2 else 0 for v in gears.values())
    print(res)

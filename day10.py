...
def is_loop(cur, start, dir, grid, res, dist=0):
    cur_pos = grid[cur[0]][cur[1]]
    dist += 1
    if cur[0] == start[0] and cur[1] == start[1]:
        return True
    # moves for down
    if (cur_pos == '|' and dir == 'T') or (cur_pos == '7' and dir == 'L') or (cur_pos == 'F' and dir == 'R'):
        res[f'{cur[0]},{cur[1]}'] = dist
        return is_loop([cur[0] + 1, cur[1]], start, 'T', grid, res, dist)
    # moves for up
    if (cur_pos == '|' and dir == 'D') or (cur_pos == 'L' and dir == 'R') or (cur_pos == 'J' and dir == 'L'):
        res[f'{cur[0]},{cur[1]}'] = dist
        return is_loop([cur[0] - 1, cur[1]], start, 'D', grid, res, dist)
    # moves for left
    if (cur_pos == '-' and dir == 'R') or (cur_pos == '7' and dir == 'D') or (cur_pos == 'J' and dir == 'T'):
        res[f'{cur[0]},{cur[1]}'] = dist
        return is_loop([cur[0], cur[1] - 1], start, 'R', grid, res, dist)
    # moves for right
    if (cur_pos == '-' and dir == 'L') or (cur_pos == 'L' and dir == 'T') or (cur_pos == 'F' and dir == 'D'):
        res[f'{cur[0]},{cur[1]}'] = dist
        return is_loop([cur[0], cur[1] + 1], start, 'L', grid, res, dist)
...
def main(inp, is_real):
    ...
    res = defaultdict(lambda: sys.maxsize)
    route = set()
    for d, cur in directions.items():
        loop_dist = {}
        if is_loop(cur, [start_x, start_y], d, grid, loop_dist):
            for k, v in loop_dist.items():
                route.add(k)
                if res[k] > v:
                    res[k] = v
    ...
    for x, row in enumerate(grid):
        ...
        for y, c in enumerate(row):
            if f'{x},{y}' in route or [x, y] == [start_x, start_y]:
                if [x, y] == [start_x, start_y]:
                    # This only works for my puzzle input
                    c = '|'
                if c == '|':
                    in_loop = not in_loop
                elif c in ('F', 'L'):
                    segment_start = c
                elif (segment_start == 'F' and c == 'J') or (segment_start == 'L' and c == '7'):
                    in_loop = not in_loop
            elif in_loop:
                res += 1
    print(res)

...
class Node:
    def __init__(self, line):
        self.source, rest = line.split(' = ')
        self.left, self.right = rest.replace('(', '').replace(')', '').split(', ')

    def __repr__(self):
        return f'Node {self.source} (left: {self.left}, right: {self.right})'
...
def main(inp, is_real):
    ...
    for line in inp[2:]:
        n = Node(line)
        nodes[n.source] = n
        if n.source.endswith('A'):
            starting_nodes.append(n.source)

    res = 1
    current = starting_nodes.copy()
    end_state_counts = [0 for x in range(len(current))]
    for dir in cycle(path):
        for i, n in enumerate(current):
            target = nodes[n].left if dir == 'L' else nodes[n].right
            current[i] = target
            if target.endswith('Z'):
                end_state_counts[i] = res
        res += 1
        if not any(x == 0 for x in end_state_counts):
            break
    print(math.lcm(*end_state_counts))
